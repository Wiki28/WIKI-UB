...
@wiki_cmd(pattern="update( now| deploy|$)")
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy Wiki-Userbot Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**😑 Wiki-Userbot Sudah Versi Terbaru**")
        return repo.__del__()
