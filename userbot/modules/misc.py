...
from userbot.utils import edit_or_reply, wiki_cmd, time_formatter
...
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Wiki-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**Wiki-Userbot Berhasil di matikan!**")
...
@Wiki_cmd(pattern="restart$")
async def restart_bot(event):
    if event.sender_id in SUDO_USERS:
        return
    await edit_or_reply(event, "**Wiki-Userbot Berhasil di Restart**")
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**Wiki-Userbot Berhasil Di Restart**"
        )
    args = [sys.executable, "-m", "userbot"]
    execle(sys.executable, *args, environ)


@wiki_cmd(pattern="readme$")
async def reedme(event):
    await edit_or_reply(
        event,
        "**Berikut sesuatu untuk kamu baca:**\n"
        "\n✣ [Userbot Repo](https://github.com/Wiki28/Wiki-Userbot/blob/Wiki-Userbot/README.md)"
        "\n✣ [Video Tutorial](https://youtu.be/tTDaPKsGC1I)"
        "\n✣ [List Variabel Heroku untuk Wiki-Userbot](https://telegra.ph/List-Variabel-Heroku-untuk-Wiki-Userbot-02-03)"
...
@wiki_cmd(pattern="repo$")
async def repo_is_here(event):
    xx = await edit_or_reply(event, "`Processing...`")
    await xx.edit(
        f"**Hey**, __I am using__ 😑 **Wiki-Userbot** 😑\n\n"
        f"      __Thanks For Using me__\n\n"
        f"✣ **Userbot Version :** `{BOT_VER}@{branch}`\n"
        f"✣ **Group Support :** [Wiki Group](t.me/sharinguserbot)\n"
        f"✣ **Wiki Channel :** [Wiki Channel](t.me/WikiTapiChannel)\n"
        f"✣ **Owner Repo :** [Wiki W](t.me/Hanya_W)\n"
        f"✣ **Repo :** [Wiki-Userbot](https://github.com/Wiki28/Wiki-Userbot)\n"
    )


@wiki_cmd(pattern="string$")
async def string_is_here(event):
    await edit_or_reply(
        event,
        "✥ **GET STRING SESSION TELEGRAM :** [KLIK DISINI](https://replit.com/@Wiki28/stringen?v=1)\n",
    )


@wiki_cmd(pattern="raw$")
async def raw(event):
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}sleep`\
        \n  •  **Function : **Biarkan Wiki-Userbot tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository Wiki-Userbot`\
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository Wiki-Userbot\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String Wiki-Userbot\
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart Wiki-Userbot`\
        \n\n  •  **Syntax :** `{cmd}restart`\
