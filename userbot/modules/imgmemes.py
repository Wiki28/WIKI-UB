...
@bot.on(wiki_cmd(outgoing=True, pattern=r"trump(?: |$)(.*)"))
...
@bot.on(wiki_cmd(pattern=r"modi(?: |$)(.*)", outgoing=True))
...
@bot.on(wiki_cmd(outgoing=True, pattern=r"cmm(?: |$)(.*)"))
async def cmm(event):
    text = event.pattern_match.group(1)
    text = re.sub("&", "", text)
    reply_to_id = event.message
...
@bot.on(wiki_cmd(outgoing=True, pattern=r"kanna(?: |$)(.*)"))
...
@bot.on(wiki_cmd(outgoing=True, pattern=r"\.tweet(?: |$)(.*)"))
...
@bot.on(wiki_cmd(pattern=r"threat(?: |$)(.*)", outgoing=True))
async def nekobot(event):
...
@bot.on(wiki_cmd(pattern=r"trash(?: |$)(.*)", outgoing=True))
async def nekobot(event):
    replied = await event.get_reply_message()
    if not os.path.isdir(TEMP_DOWNLOAD_DIRECTORY):
        os.makedirs(TEMP_DOWNLOAD_DIRECTORY)
...
@bot.on(wiki_cmd(pattern=r"trap(?: |$)(.*)", outgoing=True))
async def nekobot(e):
...
@bot.on(wiki_cmd(outgoing=True, pattern=r"fgs ((.*) ; (.*))"))
...
@bot.on(wiki_cmd(outgoing=True, pattern=r"ph(?: |$)(.*)"))
